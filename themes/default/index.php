				...
				<div id="tweets" class="rounded">
					...
					?>
						...
						<div class="<?php echo $class; ?>">
							...
							<p class="meta">
								<a href="<?php echo ((isset($single_tweet) && $single_tweet) ? 'https://x.com/'.$config['twitter']['username'].'/status/' : $config['system']['baseUrl']).$t->id; ?>/" rel="bookmark"><?php echo $t->get_date(); ?></a>
								via
								<?php echo $t->source; echo ($t->in_reply_to_status_id != 0) ? ' <a href="https://x.com/'.$t->in_reply_to_screen_name.'/status/'.$t->in_reply_to_status_id.'">in reply to '.$t->in_reply_to_screen_name.'</a>' : ''; ?>
							</p>
						</div>
